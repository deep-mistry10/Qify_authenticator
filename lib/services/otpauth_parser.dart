...
class OtpAuthParser {
  static ParsedOtpAuth parse(String raw) {
    ...
    if (uri.host.toLowerCase() != 'totp') {
      throw const FormatException('Only TOTP URIs are supported');
    }
    ...
    final label = uri.pathSegments.isEmpty
        ? ''
        : Uri.decodeComponent(uri.pathSegments.join('/')).trim();
    ...
    if (colon >= 0) {
      ...
      final labelAccount = label.substring(colon + 1).trim();
    ...
    if (digits != 6 && digits != 8) {
      throw const FormatException('Unsupported OTP digits');
    }
    if (period <= 0) throw const FormatException('Invalid period');
    if (!{'SHA1', 'SHA-1', 'SHA256', 'SHA-256', 'SHA512', 'SHA-512'}
        .contains(algorithm)) {
    ...
    return ParsedOtpAuth(
      ...
      accountName: account.isEmpty ? 'Unknown account' : account,
  ...
  static bool looksLikeOtpAuth(String value) =>
      value.trim().toLowerCase().startsWith('otpauth://totp/');

  static Map<String, String> parseAdditionalParameters(String raw) {
    final uri = Uri.tryParse(raw.trim());
    if (uri == null) return const {};
    return Map.unmodifiable(uri.queryParameters);
  }
}
